...
export class CommonService {
    ...
    importJSONResponseMessage(status: any, recordsCount: number, typeName: string) {
        const failedMessage = '<ul><li>' + status.failed.join('</li><li>') + '</li></ul>';
        if (status.success.length === recordsCount) {
            ...
            return {
                type: 'warning',
                message: `Some ${typeName} imported successfully. Failed ${typeName}:` + failedMessage,
            };
