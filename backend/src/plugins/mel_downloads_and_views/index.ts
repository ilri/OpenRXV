...
const melnumbersUrl = 'https://mel.cgiar.org/dspace/getdspaceitemsvisits';
...
export class MELDownloadsAndViews {
  ...
  async addJobs(queue, plugin_name, data, index_name: string) {
    ...
    try {
      ...
      let totalPages =
        Math.ceil((batch?.hits?.total as SearchTotalHits).value / 100 || 0) - 1;
      this.addJob(queue, plugin_name, data, batch);

      if (scrollId) {
        for (totalPages; totalPages > 0; totalPages--) {
          const nextBatch: SearchResponse =
