...
export class AddMissingItems {
  ...
  async start(queue, name: string, index_name: string, concurrency: number) {
    queue.process(name, concurrency, async (job: Job<any>) => {
      ...
      if (job.data.repo.type === 'DSpace') {
        ...
        const owningCollectionEmbed =
          'owningCollection/parentCommunity/parentCommunity/parentCommunity/parentCommunity/parentCommunity/parentCommunity/parentCommunity/parentCommunity/parentCommunity/parentCommunity';
        const mappedCollectionsEmbed =
          'mappedCollections/parentCommunity/parentCommunity/parentCommunity/parentCommunity/parentCommunity/parentCommunity/parentCommunity/parentCommunity/parentCommunity/parentCommunity';
        url = `${job.data.itemEndPoint}/${job.data.identifier}?embed=${owningCollectionEmbed},${mappedCollectionsEmbed},thumbnail`;
      }
      const result = await lastValueFrom(
        this.http.get(url).pipe(map((d) => d.data)),
      ).catch(() => null);
      ...
      if (result && result.type == 'item') {
        ...
        if (job.data.repo.type === 'DSpace') {
          ...
          formatted = this.dspaceService.formatter(
            result,
            job.data.repo.schema,
            mappingValues,
          );
        } else if (job.data.repo.type === 'DSpace7') {
          ...
          formatted = this.dspace7Service.formatter(
            result,
            job.data.repo.schema,
            mappingValues,
          );
  ...
  async getIdentifiers(repo, sitemapIdentifier): Promise<any> {
    return new Promise(async (resolve, reject) => {
      try {
        const openPointInTime: OpenPointInTimeResponse =
          await this.elasticsearchService
            .openPointInTime({
        ...
        const getMoreUntilDone = async () => {
          ...
          if (response?.hits?.hits.length > 0) {
            response.hits.hits.map((d) => {
              ...
              if (identifier) allRecords.push(identifier);
            });
  ...
  async addJobs(queue, plugin_name, data, index_name: string) {
    ...
    try {
      ...
      if (data?.sitemapIdentifier === 'handle') {
        ...
        this.logger.log(
          'sitemap Identifier is not defined for the plugin ' + repo.siteMap,
        );
        return { success: false };
      }

      if (repo.type === 'DSpace7') {
        const collectionsCommunitiesIds =
          await this.getCollectionsCommunitiesIds(repo.itemsEndPoint);

        for (let i = 0; i < collectionsCommunitiesIds.length; i++) {
          const index = sitemapIdentifiers.indexOf(
            collectionsCommunitiesIds[i],
          );
          if (index !== -1) {
      ...
      this.logger.log(
        'Items sitemap identifiers found ' + sitemapIdentifiers.length,
      );

      console.log('repo => ', JSON.stringify(repo));
      itemsIdentifiers = await this.getIdentifiers(
        repo,
        data.sitemapIdentifier,
      ).catch((e) => {
        return null;
      });
      ...
      if (queue) {
        for (let i = 0; i < sitemapIdentifiers.length; i++) {
          await queue.add(
            `${index_name}_plugins_dspace_add_missing_items`,
            {
              index: index_name,
              repo,
        ...
        this.logger.log(
          'Missing identifiers for ' + data.repo + ' added to the Queue',
        );
      } else {
        this.logger.log(
          'Missing identifiers for ' + data.repo + ' queue not found',
        );
      }
  ...
  async getCollectionsCommunitiesIds(endPoint) {
    ...
    const communitiesRequest = await lastValueFrom(
      this.http.get(
        `${endPoint}/discover/search/objects?dsoType=community&size=1&page=0`,
      ),
    ).catch(() => {
      return null;
    });
    if (
      communitiesRequest?.data?._embedded?.searchResult?.page?.totalElements &&
      communitiesRequest.data._embedded.searchResult.page.totalElements > 0
    ) {
      const totalPages = Math.ceil(
        communitiesRequest.data._embedded.searchResult.page.totalElements / 100,
      );
      for (let page = 0; page < totalPages; page++) {
        const data = await lastValueFrom(
          this.http.get(
            `${endPoint}/discover/search/objects?dsoType=community&size=100&page=${page}`,
          ),
        ).catch(() => {
          return null;
        });
        const communities =
          data?.data?._embedded?.searchResult?._embedded?.objects;
        if (communities) {
    ...
    const collectionsRequest = await lastValueFrom(
      this.http.get(
        `${endPoint}/discover/search/objects?dsoType=collection&size=1&page=0`,
      ),
    ).catch(() => {
      return null;
    });
    ...
    if (
      collectionsRequest?.data?._embedded?.searchResult?.page?.totalElements &&
      collectionsRequest.data._embedded.searchResult.page.totalElements > 0
    ) {
      const totalPages = Math.ceil(
        collectionsRequest.data._embedded.searchResult.page.totalElements / 100,
      );
      for (let page = 0; page < totalPages; page++) {
        const data = await lastValueFrom(
          this.http.get(
            `${endPoint}/discover/search/objects?dsoType=collection&size=100&page=${page}`,
          ),
        ).catch(() => {
          return null;
        });
