...
export class IndexMetadataService extends ElasticService {
  ...
  async DSpaceMetadataAutoRetrieve(link) {
    ...
    if (Number(version?.apiVersion) > 0) {
      ...
      const base = await lastValueFrom(
        this.httpService.get(`${link}/items?limit=25`).pipe(map((d) => d.data)),
      ).catch(() => null);
  ...
  async DSpace7MetadataAutoRetrieve(link) {
    ...
    const base = await lastValueFrom(
      this.httpService
        .get(
          `${link}/discover/search/objects?dsoType=item&embed=thumbnail,owningCollection,mappedCollections&size=25`,
        )
        .pipe(map((d) => d.data)),
    ).catch(() => null);
      ...
      base._embedded.searchResult._embedded.objects.map((item) => {
        ...
        baseFields.map((baseField) => {
          if (['metadata', '_links', '_embedded'].indexOf(baseField) === -1) {
            ...
            const expandedFields = Object.keys(
              item._embedded.indexableObject[baseField],
            );
    ...
    if (
      schemasPages?.page?.totalElements &&
      schemasPages.page.totalElements > 0
    ) {
