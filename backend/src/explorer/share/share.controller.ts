...
export class ShareController {
  ...
  @HttpCode(200)
  @Post('/:dashboard_name')
  async save(@Param('dashboard_name') dashboard_name: string = 'DEFAULT_DASHBOARD', @Body() query: any) {
    const dashboard = await this.jsonFilesService.GetDashboard(dashboard_name);
    const index_name = dashboard?.name ? `${dashboard.name}-shared` : this.shareservice.index;
    return await this.shareservice.saveShare(query, index_name);
  }

  @HttpCode(200)
  @Get('/:dashboard_name')
  async getAll(@Param('dashboard_name') dashboard_name: string = 'DEFAULT_DASHBOARD') {
    const dashboard = await this.jsonFilesService.GetDashboard(dashboard_name);
    const index_name = dashboard?.name ? `${dashboard.name}-shared` : this.shareservice.index;
  ...
  async get(@Param('dashboard_name') dashboard_name: string = 'DEFAULT_DASHBOARD', @Param('id') id: string) {
    const dashboard = await this.jsonFilesService.GetDashboard(dashboard_name);
    const index_name = dashboard?.name ? `${dashboard.name}-shared` : this.shareservice.index;
