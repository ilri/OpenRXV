...
export class ExportService {
  ...
  async downloadFile(
    ...
    try {
      ...
      if (type == 'docx') {
        filePath = await this.createDocx(
          fileNameg,
          hits,
          query,
          part,
        ...
        filePath = (await this.createPdf(
          await this.createDocx(fileNameg, hits, query, part, websiteName),
          fileName,
          type,
          res,
          response,
      ...
      if (filePath) response.fileName = filePath;
      res.json(response);
    } catch (error) {
      ...
      if (query.query.hasOwnProperty('bool')) {
        if (query.query.bool.hasOwnProperty('filter'))
          if (query.query.bool.filter.hasOwnProperty('term'))
            select = JSON.stringify(query.query.bool.filter.term)
              .replace(/[^-A-Za-z,: ]/g, '')
              .replace('keyword', '');
        if (query.query.bool.hasOwnProperty('must')) {
          ...
          if (query.query.bool.must.hasOwnProperty('filter'))
            select =
              ',' +
              select +
              JSON.stringify(query.query.bool.filter.term)
                .replace(/[^-A-Za-z,: ]/g, '')
                .replace('keyword', '') +
              ',';
          if (query.query.bool.must.hasOwnProperty('range'))
            select =
              '_' +
              select +
              JSON.stringify(query.query.bool.must.range)
                .replace(/[^A-Za-z 0-9 _,: ]/g, '')
                .replace('keyword', '') +
              '_';
    ...
    try {
      const zip = new PizZip(
        await fs.promises.readFile(
          this.resolvePath(fileName + '', false),
          'binary',
        ),
      );
