...
export class DSpace7Service {
  ...
  async getMappingValues(index_name, forceGet) {
    if (forceGet || !this.mappingValues.hasOwnProperty(index_name)) {
      this.mappingValues[index_name] =
        await this.formatService.getMappingValues(index_name);
    }
  ...
  async RegisterProcess(queue) {
    queue.process(this.harvesterName, 0, async (job: Job<any>) => {
      ...
      job.data.repo.itemsEndPoint = job.data.repo.itemsEndPoint.replace(
        /\/$/gm,
        '',
      );
      ...
      try {
        ...
        const request = await lastValueFrom(this.http.get(url)).catch((d) => {
    ...
    for (const harvested of data) {
      if (harvested?._embedded?.indexableObject) {
        ...
        const formatted = this.formatter(
          ...
          this.mappingValues[job.data.repo.index_name],
        );
          ...
          if (formatted[spleted[1]]) {
            if (typeof formatted[spleted[1]] === 'string')
              formatted[job.data.repo.years] =
                formatted[spleted[1]].split('-')[0];
            if (
              Array.isArray(formatted[spleted[1]]) &&
              typeof formatted[spleted[1]][0] === 'string'
            )
              formatted[job.data.repo.years] =
                formatted[spleted[1]][0].split('-')[0];
          }
        }
        formatted['id'] = item.uuid ? item.uuid.toString() : item.id.toString();
        formatted['repo'] = job.data.repo.name;
        finalData.push({
          index: { _index: job.data.repo.index_name + '_temp' },
        });
  ...
  async addJobs(queue, repository) {
    ...
    try {
      ...
      let page_number =
        repository?.startPage && Number(repository.startPage) >= 0
          ? Number(repository.startPage)
          : 0;
  ...
  formatter(harvestedItem: any, schemas: any, mappingValues: any) {
    ...
    _.each(schemas, (schemaItems: any, schemaName: string) => {
      if (_.isArray(schemaItems)) {
              ...
              if (harvestedItem[schemaName].hasOwnProperty(metadataFieldName)) {
                if (metadataFieldName === schemaItem.where[schemaKeyName]) {
                  ...
                  const metadataElements =
                    harvestedItem[schemaName][metadataFieldName];
                  metadataElements.map((metadataElement) => {
                    if (metadataElement.hasOwnProperty(schemaValueName)) {
                      const mappedValue = this.formatService.mapIt(
                        metadataElement[schemaValueName],
                        addOn,
                        schemaItem.value.value,
                        mappingValues,
                      );
                      const value = schemaItem.prefix
                        ? schemaItem.prefix + mappedValue
                        : mappedValue;
                      if (value !== '' && value != null) {
                        values.push(value);
                      }
        ...
        if (harvestedItem.hasOwnProperty(schemaName)) {
          // These are item basic information (id, name, handle, archived, ...)
          const mappedValue = this.formatService.mapIt(
            harvestedItem[schemaName],
            addOn,
            null,
            mappingValues,
          );
          ...
          let embeddedItem =
            harvestedItem?._embedded &&
            harvestedItem._embedded.hasOwnProperty(schemaName)
              ? harvestedItem._embedded[schemaName]
              : null;

          if (
            embeddedItem?._embedded &&
            embeddedItem._embedded.hasOwnProperty(schemaName)
          ) {
          ...
          if (
            embeddedItem &&
            !_.isArray(embeddedItem) &&
            _.isObject(embeddedItem)
          ) {
              ...
              const mappedValues = embeddedItem
                .map((metadataElement: any) => {
                  if (metadataElement?._embedded?.parentCommunity) {
                    communities = this.formatService.extractParentCommunities(
                      metadataElement._embedded.parentCommunity,
                      communities,
                      communityTitleFieldName,
                    );
                  }
                  return this.formatService.mapIt(
                    metadataElement[titleFieldName],
                    addOn,
                    null,
                    mappingValues,
                  );
